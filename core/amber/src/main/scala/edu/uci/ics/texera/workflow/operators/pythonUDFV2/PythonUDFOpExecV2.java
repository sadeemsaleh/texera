...
public class PythonUDFOpExecV2 implements OperatorExecutor {
    ...
    private Queue<Tuple> inputTupleBuffer = new LinkedList<>();
    private Queue<Tuple> outputTupleBuffer = new LinkedList<>();

    PythonUDFOpExecV2(String pythonScriptText, String pythonScriptFile, String arguments, int batchSize) {
    ...
    public void open() {
        ...
        try {
            flightClient.doAction(new Action("open", this.arguments.getBytes())).next().getBody();
        } catch (Exception e) {
            e.printStackTrace();
            try {
                if (isDynamic) deleteTempFile(pythonScriptPath);
    ...
    public Iterator<Tuple> processTexeraTuple(Either<Tuple, InputExhausted> tuple, int input) {
        if (tuple.isLeft()) {
            ...
            if (inputTupleBuffer.size() == batchSize) {
                ...
                return JavaConverters.asScalaIterator(processTuple(false));
            }
        }
        else {
            outputTupleBuffer = new LinkedList<>();
            return JavaConverters.asScalaIterator(Stream.concat(
                    // There are possible unprocessed tuples, finish them.
                    Stream.generate(() -> processTuple(false)).flatMap(iter -> Streams.stream(iter)),
                    // send input exhausted signal
                    Stream.generate(() -> processTuple(true)).flatMap(iter -> Streams.stream(iter))
            ).iterator());
        }
        return JavaConverters.asScalaIterator(null);
    }

    private java.util.Iterator<Tuple> processTuple(boolean inputExhausted) {
        try {
            if (inputExhausted) {
                // trigger input exhausted in python side
                byte[] resultBytes = flightClient.doAction(new Action("inputExhausted")).next().getBody();
                Map<String, String> result = globalObjectMapper.readValue(resultBytes,
                ...
                // trigger process tuple in python side
                byte[] resultBytes = flightClient.doAction(new Action("processTuple")).next().getBody();
                Map<String, String> result = globalObjectMapper.readValue(resultBytes,
                        new TypeReference<Map<String,String>>() {});
                if (result.get("status").equals("Fail")) {
                    String errorMessage = result.get("errorMessage");
            ...
            return new java.util.Iterator<Tuple>() {

                private java.util.Iterator<Tuple> outputIterator = null;
                ...
                public boolean hasNext() {
                    ...
                    try {
                        byte[] resultBytes = flightClient.doAction(new Action("getNext")).next().getBody();
                        Map<String, String> result = globalObjectMapper.readValue(resultBytes,
                                new TypeReference<Map<String,String>>() {});
                        if (result.get("status").equals("Fail")) {
                            String errorMessage = result.get("errorMessage");
                            throw new RuntimeException(errorMessage);
                        } else if (result.get("status").equals("StopIteration")) {
                            // fetch output data of the entire batch back from python
                            Queue<Tuple> output = new LinkedList<>();
                            readArrowStream(flightClient, "fromPython", output);
                            this.outputIterator = output.iterator();
    ...
    private static String getPythonResourcePath(String fileName) {
        ...
        return Utils.amberHomePath().resolve("src/main/python/udf_v2").resolve(fileName).toString();
    }
