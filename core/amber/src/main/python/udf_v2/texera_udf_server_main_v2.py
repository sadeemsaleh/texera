...
class UDFServer(pyarrow.flight.FlightServerBase):
	def __init__(self, udf_op, host="localhost", flight_location=None, tls_certificates=None, auth_handler=None):
		super(UDFServer, self).__init__(location, auth_handler, tls_certificates)
		self.flights = {}
		self.host = host
		self.tls_certificates = tls_certificates
		self.udf_op = udf_op
		self.udf_output_generator = None
	...
	def process_batch(self, input_dataframe):
		if input_dataframe == "InputExhausted":
			yield from self.udf_op.process_tuple(input_dataframe)
		else:
			for index, row in input_dataframe.iterrows():
				yield from self.udf_op.process_tuple(row)
...
if __name__ == '__main__':
	...
	if FinalUDF is None:
		if map_func is not None:
			FinalUDF = TexeraMapOperatorV2(map_func)
		else:
			FinalUDF = TexeraFilterOperatorV2(filter_func)
